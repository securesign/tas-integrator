# Jenkins detailed scan reference
...
| `agent_version` | Version from [.claude-plugin/plugin.json (plugin version metadata) |
...
`{{ci_platform}}` to `Jenkins` and retain the Jenkins-specific signing and
verification snippets below.
